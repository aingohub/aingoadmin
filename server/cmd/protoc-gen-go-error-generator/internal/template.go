...
var (
{{ range .Errors }}
	{{.EnumValue}}            = &{{ .ErrorStructName }}{ {{ .CodeFieldName }}: {{ .Code }}, {{ .MsgFieldName }}: "{{ .Msg }}"} //{{ .Msg }}
{{- end }}
)
`
...
{{ range $key, $value := .MapInfo }}
  {{ $key }} = map[int32]*{{ $errorStructName }}{
    {{- range $value }}
         {{ .Code }}: &{{ $errorStructName }}{ {{ .CodeFieldName }}: {{ .Code }}, {{ .MsgFieldName }}: "{{ .Msg }}"},
    {{- end }}
  }
{{ end }}
)
