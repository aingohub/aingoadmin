...
func newUser(db *gorm.DB, opts ...gen.DOOption) user {
	...
	_user.UserProfile = userHasOneUserProfile{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("UserProfile", "model.UserProfile"),
	}
...
type user struct {
	...
	UserProfile userHasOneUserProfile
...
type userHasOneUserProfile struct {
	db *gorm.DB

	field.RelationField
}

func (a userHasOneUserProfile) Where(conds ...field.Expr) *userHasOneUserProfile {
	if len(conds) == 0 {
		return &a
	}
...
func (a userHasOneUserProfile) WithContext(ctx context.Context) *userHasOneUserProfile {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a userHasOneUserProfile) Session(session *gorm.Session) *userHasOneUserProfile {
	a.db = a.db.Session(session)
	return &a
}

func (a userHasOneUserProfile) Model(m *model.User) *userHasOneUserProfileTx {
	return &userHasOneUserProfileTx{a.db.Model(m).Association(a.Name())}
}

type userHasOneUserProfileTx struct{ tx *gorm.Association }

func (a userHasOneUserProfileTx) Find() (result *model.UserProfile, err error) {
	return result, a.tx.Find(&result)
}

func (a userHasOneUserProfileTx) Append(values ...*model.UserProfile) (err error) {
...
func (a userHasOneUserProfileTx) Replace(values ...*model.UserProfile) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasOneUserProfileTx) Delete(values ...*model.UserProfile) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasOneUserProfileTx) Clear() error {
	return a.tx.Clear()
}

func (a userHasOneUserProfileTx) Count() int64 {
	return a.tx.Count()
}
