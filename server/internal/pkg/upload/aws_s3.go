...
func (*AwsS3) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	filename := config.C.Upload.AwsS3.PathPrefix + "/" + fileKey
	...
	_, err := uploader.Upload(&s3manager.UploadInput{
		Bucket: aws.String(config.C.Upload.AwsS3.Bucket),
		Key:    aws.String(filename),
		Body:   f,
	})
	if err != nil {
		logging.Error("function uploader.Upload() Filed", zap.Any("err", err.Error()))
		return "", "", err
	}

	return config.C.Upload.AwsS3.BaseURL + "/" + filename, fileKey, nil
}
...
func (*AwsS3) DeleteFile(key string) error {
	session := newSession()
	svc := s3.New(session)
	filename := config.C.Upload.AwsS3.PathPrefix + "/" + key
	bucket := config.C.Upload.AwsS3.Bucket
	...
	if err != nil {
		logging.Error("function svc.DeleteObject() Filed", zap.Any("err", err.Error()))
...
func newSession() *session.Session {
	sess, _ := session.NewSession(&aws.Config{
		Region:           aws.String(config.C.Upload.AwsS3.Region),
		Endpoint:         aws.String(config.C.Upload.AwsS3.Endpoint), //minio在这里设置地址,可以兼容
		S3ForcePathStyle: aws.Bool(config.C.Upload.AwsS3.S3ForcePathStyle),
		DisableSSL:       aws.Bool(config.C.Upload.AwsS3.DisableSSL),
		Credentials: credentials.NewStaticCredentials(
			config.C.Upload.AwsS3.SecretID,
			config.C.Upload.AwsS3.SecretKey,
			"",
		),
	})
	return sess
