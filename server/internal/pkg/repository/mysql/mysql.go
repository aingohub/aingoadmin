...
func New() error {
	cfg := config.Get().Storage.MySQL
	dbr, errR := dbConnect(cfg.Read.User, cfg.Read.Pass, cfg.Read.Addr, cfg.Read.Name)
	if errR != nil {
		return errR
	}

	dbw, errW := dbConnect(cfg.Write.User, cfg.Write.Pass, cfg.Write.Addr, cfg.Write.Name)
	if errW != nil {
		return errW
	}
