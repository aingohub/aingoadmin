...
import (
	...
	resp "gin-api-mono/internal/response"
...
func New(logger *zap.Logger, options ...Option) (Mux, error) {
	...
	mux.engine.Use(func(ctx *gin.Context) {
		...
		defer func() {
			...
			// endregion
			languageStr := ctx.Request.Header.Get("language")
			// region 发生错误，进行返回
			if ctx.IsAborted() {
				...
				if err := context.abortError(); err != nil { // customer err
					...
					if languageStr == "en-US" {
						if e, ok := errcode.Msg_English[int32(businessCode)]; !ok {
							businessCodeMsg = e.Msg
						}
					}
					response = &resp.Result{
						Success: false,
						Code:    int64(businessCode),
			...
			if response != nil {
				ctx.JSON(http.StatusOK, &resp.Result{
					Code:    int64(errcode.SUCCESS.Code),
